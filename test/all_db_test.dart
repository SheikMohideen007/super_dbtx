...
// import 'package:super_sdbtx/super_sdbtx.dart';
...
//     await DBUtils.clearTable(sdb, 'users');
//     final rc = await DBUtils.getRowCount(sdb, 'users');
//     expect(rc, 0);
//   });
...
//     final tables = await DBUtils.fetchTableNames(sdb);
//     expect(tables.contains('users'), true);

//     final cols = await DBUtils.getTableColumns(sdb, 'users');
//     expect(cols.contains('id'), true);
//     expect(cols.contains('name'), true);

//     final path = await DBUtils.getDBPath(sdb);
//     expect(path, isNotEmpty);
//   });

//   test('VACUUM works and getDatabaseSize', () async {
//     await DBUtils.vacuum(
//       sdb,
//     ); // in-memory does not change file size but must not crash
//     // getDatabaseSize needs a file DB - test that it returns int for a temporary file (skip for in-memory)
//     final fileDbPath = await getDatabasesPath() + '/test_size.sdb';
...
//       },
//     );
//     await fileDb.insert('users', {'name': 'Z'});
//     await fileDb.close();
//     final size = await DBUtils.getDatabaseSize(fileDbPath);
//     expect(size >= 0, true);
//     // cleanup
//     final f = File(fileDbPath);
//     if (await f.exists()) await f.delete();
//   });

//   test('addColumn & renameColumn flow', () async {
//     // add column
//     await DBUtils.addColumn(sdb, 'users', 'age INTEGER DEFAULT 0');
//     var cols = await DBUtils.getTableColumns(sdb, 'users');
//     expect(cols.contains('age'), true);

//     // rename column (if supported)
//     await DBUtils.renameColumn(sdb, 'users', 'age', 'years');
//     cols = await DBUtils.getTableColumns(sdb, 'users');
