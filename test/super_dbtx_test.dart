...
void main() {
  ...
  test('Drop table using DBUtils', () async {
    expect(await DBUtils.tableExists(sdb, 'users'), true);

    await DBUtils.dropTable(sdb, 'users');

    final exists = await DBUtils.tableExists(sdb, 'users');
    expect(exists, false);
  });

  test('VACUUM does not crash in memory DB', () async {
    await DBUtils.vacuum(sdb);
  });
}
